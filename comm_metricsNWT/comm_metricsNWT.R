...
defineModule(sim, list(
  ...
  parameters = rbind(
    ...
    defineParameter(".saveInitialTime", "numeric", 10, NA, NA, "This describes the simulation time at which the first save event should occur"),
...
diversityStats <- function(sim){
 ...
 #browser()
  sim$diversityByPolygon[[as.character(time(sim))]] <- extract(sim$currentDiversityRasters, sim$caribouArea1, fun=mean, na.rm=TRUE)
  return(invisible(sim))
}
...
Save <- function(sim){
  ...
  fname <- file.path(dir,sprintf("%03d.%s",as.integer(time(sim)),".tif"))
